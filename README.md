...
## Deploy — Neon + Render (backend + frontend)
...
manzilini talab qiladi. Backend va frontend ikkalasi ham `render.yaml`
orqali bitta Blueprint sifatida joylanadi.
...
### 3. Render — Next.js frontend

`render.yaml`dagi ikkinchi xizmat (`qaraqalpaqstan-web`) shu maqsadda —
Blueprint'ni ulaganda ikkalasi birga taklif qilinadi.

1. Muhit o'zgaruvchisi: `NEXT_PUBLIC_API_URL` = backend manzili
   (masalan `https://qaraqalpaqstan-api.onrender.com`, oxirida `/` yo'q).
2. Deploydan so'ng frontend bergan manzilni backend'dagi `CORS_ORIGINS`ga
   qo'shing va `qaraqalpaqstan-api`ni qayta ishga tushiring.

> `NEXT_PUBLIC_*` qiymatlari **build vaqtida** kodga o'rnatiladi — uni
> o'zgartirgach frontend xizmatini qayta deploy qilish shart.

### Bepul darajaning cheklovi

Render'ning bepul xizmati 15 daqiqa harakatsizlikdan keyin uxlaydi va
uyg'onishi ~30–60 soniya oladi — bu ikkala xizmatga (backend va frontend)
ham tegishli, ya'ni ikkalasi mustaqil uyquga ketishi mumkin. Sahifa
ochilganda backendga "uyg'otuvchi" so'rov yuboriladi (`warmUpApi`), chat
esa 20 soniya kutadi va kutish tugasa holatni ochiq aytadi. Frontend
o'zi uxlab qolgan bo'lsa, birinchi so'rov shunchaki sekin yuklanadi.
#   Q a r a q a l p a q s t a n - J I -  
 #   Q a r a q a l p a q s t a n - J I -  
 