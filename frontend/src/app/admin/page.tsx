        ...
        <div className="mx-auto w-full max-w-[1400px] flex-1 px-4 py-5">
          {error && (
            <div className="mb-4 flex items-start gap-2 rounded-2xl bg-crimson/12 px-4 py-3 ring-1 ring-crimson/30">
              ...
              <span className="text-[13.5px] leading-relaxed text-coral">{error}</span>
            </div>
          )}
            ...
            {tiles.map((s, i) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <div className="truncate text-[12px] text-ink-3">{s.label}</div>
...
function BaseState({ summary }: { summary: AdminSummary | null }) {
  if (!summary) {
    return <div className="py-8 text-center text-[13.5px] text-ink-3">Maǵlıwmat joq</div>;
  }
              ...
              {summary.categories.map((c) => (
                <tr key={c.id} className="border-t border-hairline/50">
                  <td className="px-3 py-2">
                    ...
                    <div className="text-[11.5px] text-ink-3">{c.source_dir}</div>
                  </td>
          ...
          {summary.modules.map((m) => (
            <div key={m.id} className="rounded-xl bg-abyss/50 px-3 py-2.5 ring-1 ring-edge/40">
              <div className="flex items-center gap-2">
                ...
                <span className="ml-auto text-[11.5px] text-ink-3">#{m.indicator_id}</span>
              </div>
              <div className="mt-1 truncate text-[12px] text-ink-3" title={m.indicator_name}>
                {m.indicator_name}
              </div>
              <div className="truncate text-[11.5px] text-ink-3">{m.unit}</div>
            </div>
          ))}
        </div>
