...
/**
 * Siyasiy kartadaǵı hár bir rayon ushın óz-ózinshe pastel reń — 17 rayon,
 * 17 qaytalanbaytuǵın Tailwind reńi (spektr boyınsha), sonlıqtan hesh bir
 * eki rayon (qońsı bolsın-bolmasın) bir reńde bolmaydı.
 */
...
export function KarakalpakstanMap({
  ...
  /** Chapta ashıq qaldırılatuǵın orın (mısalı, súzgish rail). Ádette RAIL_LEFT. */
  leftInset?: number;
  /** Ońda ashıq qaldırılatuǵın orın. Ádette fokus halına baylanıslı RAIL_RIGHT. */
  rightInset?: number;
  /** Suzuvchi boshqaruvlar júklenetuǵın joqarǵı shegara. */
  topInset?: number;
  /** Rayon ústinen ótkende shıǵatuǵın maǵlıwmat oynasha. Bas bette óshirilgen. */
  showTooltip?: boolean;
} = {}) {
  const {
    moduleId,
  ...
  const railLeft = leftInset ?? RAIL_LEFT;
  const railRightPx = rightInset ?? railRight(focusMode);
  ...
  useEffect(() => {
    ...
    const shiftPx = isWide ? stageOffsetX(size.w, focusMode, railLeft, railRightPx) : 0;
  ...
  }, [selected, focusMode, isWide, size.w, renderScale, reduce, camX, camY, camScale, railLeft, railRightPx]);
  ...
  const onPointerDown = useCallback((e: React.PointerEvent) => {
    ...
    // Pointer capture bul jerde ATAYLAP qoyılmaydı: erte qoysaq, brauzer
    // keyingi "click"ti address rayon <path>'inen emes, usı wrapper'den
    // shıqqandай esaplap, tuman tańlaw ıqtimalı toqtap qaladı. Sonlıqtan
    // capture tek chınıǵı súyrew baslanǵanda (onPointerMove'da) qoyıladı.
  }, []);
  ...
  const chrome = isWide ? { left: railLeft + 20, right: railRightPx + 20 } : { left: 12, right: 12 };
  ...
  return (
    ...
    >
          ...
          {DISTRICTS.map((d, i) => {
            ...
            return (
              <motion.path
                key={d.id}
                d={d.d}
                className={`district-path ${DISTRICT_FILLS[i % DISTRICT_FILLS.length]}`}
                tabIndex={0}
                role="button"
                aria-label={
                ...
                fillOpacity={isHover || isSelected ? 0.95 : off ? 0.2 : 0.72}
            ...
            {DISTRICTS.map((d) => {
              ...
              return (
                <g key={`edge-${d.id}`}>
                  ...
                  <path
                    d={d.d}
                    className="district-edge stroke-gray-600"
      ...
      <DistrictTooltip
        districtId={
          showTooltip && !dragging && hoveredDistrict && hoveredDistrict !== selectedDistrict
            ? hoveredDistrict
            : null
        }
        x={pointer.x}
        y={pointer.y}
        year={year}
        minX={isWide ? railLeft : 0}
        maxX={isWide ? size.w - railRightPx : size.w}
      />
      ...
      >
        ...
        {/* Legenda */}
        <div className="glass-chip absolute bottom-3 left-1/2 flex w-[min(96%,720px)] -translate-x-1/2 items-center gap-4 rounded-2xl px-5 py-3">
          <div className="flex shrink-0 items-center gap-2 border-r border-hairline/60 pr-4">
            <Crosshair size={14} className="text-ink-3" />
            <span className="text-[11px] font-semibold tracking-wider whitespace-nowrap text-ink-3 uppercase">
          ...
          <div className="flex min-w-0 flex-1 flex-wrap items-center justify-between gap-x-4 gap-y-1.5">
            {VOLUME_LEGEND.map((s) => (
              <span key={s.label} className="inline-flex items-center gap-2">
                <span
                  className="size-3 shrink-0 rounded-[4px] ring-1 ring-void/70"
                  style={{ background: s.color }}
                />
                {/* Rang yolg'iz ma'no tashimasligi uchun har bosqichda
                    o'sha bosqichga to'g'ri keladigan qiymat yoziladi */}
                <span className="tnum text-[12.5px] font-medium whitespace-nowrap text-ink-2">
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2 px-3.5 py-3">
        ...
        <button
          onClick={onClose}
          className="grid size-7 shrink-0 place-items-center rounded-md text-ink-3 transition hover:bg-raised/60 hover:text-ink"
        >
          <X size={14} />
        </button>
      </div>
      <div className="flex items-end justify-between border-t border-hairline/60 px-3.5 py-2.5">
        <div className="min-w-0">
          <div className="text-[11.5px] tracking-wider text-ink-3 uppercase">
