...
export function TaskBoard() {
  ...
  const visible = useMemo(
    () =>
      tasks
        .filter((t) => filter === "all" || taskStatus(t.progress, t.deadline).id === filter)
  ...
  const counts = useMemo(() => {
    ...
    for (const t of tasks) {
      const g = taskStatus(t.progress, t.deadline).id;
      c[g] = (c[g] ?? 0) + 1;
    }
    return c;
  }, [tasks]);
  ...
  return (
    <div className="space-y-4">
      ...
      {demoMode && (
        <div className="flex items-center gap-2 rounded-xl bg-cyan/10 px-3 py-2 text-[13px] font-medium text-cyan ring-1 ring-cyan/25">
          <span className="size-1.5 shrink-0 animate-pulse rounded-full bg-cyan" />
          Demo kórinis — server jetimsiz, kórsetilgen maǵlıwmatlar úlgi retinde
        </div>
      )}
      ...
      {!loading && visible.length === 0 && (
        <div className="rounded-2xl bg-abyss/50 px-4 py-10 text-center text-[14px] text-ink-3 ring-1 ring-edge/40">
...
function TaskCard({
  ...
  return (
    <motion.article
      ...
      className="glass flex flex-col gap-3.5 rounded-2xl p-[18px]"
    >
      <div className="flex items-start gap-2.5">
        <span className="mt-1 size-2 shrink-0 rounded-full" style={{ background: color }} />
        <h3 className="min-w-0 flex-1 text-[14px] leading-snug font-semibold text-ink">
          {task.title}
        </h3>
      </div>
      ...
      <div
        className={cn(
          "flex w-full items-center justify-center rounded-xl px-3 py-2.5 text-[13px] font-semibold tracking-wide",
          status.buttonClass,
        )}
      >
        {status.label}
