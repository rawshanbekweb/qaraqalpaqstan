    ...
    <div className="space-y-2 px-1 pt-1">
      {spec.data.map((row, i) => {
        ...
        return (
          ...
          >
            <span className="truncate text-[12px] text-ink-2">{String(row.label)}</span>
            ...
            <span className="tnum text-right text-[12px] font-semibold text-ink">{trim(a)}%</span>
          </motion.div>
        );
      })}
    </div>
    ...
    <div className="space-y-1.5 px-1 pt-1.5">
      {spec.data.map((row, i) => {
        ...
        return (
          ...
          >
            <span className="truncate text-[12px] text-ink-2" title={String(row.label)}>
              {String(row.label)}
            </span>
            ...
            <span className="tnum text-right text-[12px] font-semibold text-ink-2">
              {v > 0 ? "+" : ""}
              {trim(v)}
              {spec.unit ? spec.unit : ""}
            </span>
    ...
    >
      <figcaption className="mb-2 flex items-start gap-2">
        <div className="min-w-0 flex-1">
          ...
          {spec.subtitle ? (
            <div className="truncate text-[12px] text-ink-3">{spec.subtitle}</div>
          ) : null}
        </div>
        <button
