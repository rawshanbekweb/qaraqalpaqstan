...
import { Activity, Minimize2, TrendingDown, TrendingUp } from "lucide-react";
...
export function StatsPanel() {
  const { moduleId, year, selectedDistrict, focusMode, exitFocus } = useDashboard();
    ...
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex items-center gap-2.5 px-4 pt-3.5 pb-3">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="truncate text-[14.5px] font-bold tracking-tight text-ink">
        ...
        {focusMode && (
          <button
            ...
            className="grid size-8 shrink-0 place-items-center rounded-lg text-ink-3 transition hover:bg-raised/60 hover:text-ink"
          >
            <Minimize2 size={15} />
          </button>
        )}
      ...
      <div className="thin-scroll min-h-0 flex-1 space-y-3 overflow-y-auto px-3.5 pb-4">
        <HeroFigure
        ...
        <div className={cn("grid gap-3", deep ? "grid-cols-4" : "grid-cols-2")}>
          <StatTile
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-2.5">
                {profile.modules.map((m, i) => (
                  ...
                  >
                    ...
                    <span className="w-20 shrink-0 truncate text-[12.5px] text-ink-2">{m.name}</span>
                    ...
                    <span className="tnum w-11 shrink-0 text-right text-[12.5px] font-semibold text-ink">
                      {trim(m.share ?? 0)}%
                    </span>
                    <span
                      className="tnum inline-flex w-14 shrink-0 items-center justify-end gap-0.5 text-[11.5px]"
        ...
        {!profile && movers.length > 0 && (
          <div className="glass rounded-2xl p-3">
            <div className="mb-2 text-[14px] font-semibold text-ink">Dıqqat talap etedi</div>
              ...
              {movers
                ...
                .map((d, i) => (
                  <motion.div
                    ...
                    className="flex items-center gap-2.5 rounded-xl bg-abyss/50 px-3 py-2.5 ring-1 ring-edge/40"
                  >
                    <span
