...
export function StatTile({
  ...
  return (
    <motion.div
      ...
      className="glass relative shrink-0 overflow-hidden rounded-2xl px-3.5 py-3"
    >
      ...
      {delta !== undefined ? (
        <div className="mt-2 flex items-center gap-1.5">
          ...
          {deltaLabel ? <span className="text-[11.5px] text-ink-3">{deltaLabel}</span> : null}
        </div>
      ) : null}
...
function Sparkline({ points, accent }: { points: number[]; accent: string }) {
  ...
  return (
    <svg
      viewBox={`0 0 ${w} ${h + 4}`}
      className="mt-2.5 h-7 w-full pr-1 overflow-visible"
...
export function HeroFigure({
  ...
  return (
    <div
      className={cn(
        "glass glass-top-glow relative min-h-[150px] shrink-0 overflow-hidden rounded-2xl px-5 py-4",
        className,
      )}
    >
      <div
