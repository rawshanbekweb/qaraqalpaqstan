...
import { isEmptyResponse, mockForPath } from "@/lib/mock";
...
export function fetchStats<T>(path: string, params?: Record<string, string | number | undefined>) {
  ...
  if (!hit) {
    hit = fetch(url, { signal: AbortSignal.timeout(20_000) })
      .then((r) => {
        if (!r.ok) throw new Error(`${r.status} ${url}`);
        return r.json() as Promise<T>;
      })
      .then((data) => {
        // Server juwap berdi, biraq bazası bos bolsa — demo maǵlıwmatlar
        // menen almastırıladı, panel bos qalmasın dep.
        if (isEmptyResponse(path, data)) {
          const mock = mockForPath(path, params);
          if (mock !== undefined) return mock as T;
        }
        return data;
      })
      .catch((err: unknown) => {
        // Backend qosılmaǵan yamasa jetimsiz bolsa — demo maǵlıwmatlar
        // menen almastırıladı, panel bos qalmasın dep.
        const mock = mockForPath(path, params);
        if (mock !== undefined) return mock as T;
        throw err;
